...
eq(has_line_background(vim.api.nvim_win_get_buf(state.windows.code), "local M = {}", "CodeReaderActiveLine"), true, "source active range remains highlighted without dimming")
eq(code_reader.toggle_dimming(true), true, "dimming api enables dimming")
eq(has_highlight(vim.api.nvim_win_get_buf(state.windows.code), "return M", "CodeReaderDimLine"), true, "source dimming api restores dimming")
