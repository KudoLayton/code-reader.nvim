...
4. Give every stage a question, trigger, state model, responsibility owner, failure model, and numbered diff evidence. The renderer presents each state change as a labelled card: each card must name one changing state bearer, one owner, and that bearer's before/cause/after/invariant. Keep independent cards together when they answer one reader question, but split cards whenever the subject, owner, lifecycle, invariant, or evidence rationale can differ. A single aggregate subject may describe coupled fields only under one owner and lifecycle; never join separate subjects or owners with a conjunction or list. Use one `path#Hn` target per evidence item; use `@old` or `@new` focused ranges only when that side is the stage's primary story.
