...
9. Run the shared validator with `--emit-page-inventory`. It bootstraps only registered, lock-pinned static-analysis dependencies, resolves old/new source where possible, and records `hunk_fallback` rather than inventing a source range when resolution fails.
10. When the inventory reports an immediate split, narrow the focused ranges and split the page before requesting any reviewer. An unresolved source or unavailable static analysis is not a pass.
11. Ask one read-only subagent to perform `Page Scope Review v1` from `../../references/code-reader-markdown-format.md`, including the hunk-to-page coverage table, any `hunk_fallback` evidence, and a `focus_alignment` record for every Diff page. Treat `MISMATCH` or `INSUFFICIENT_EVIDENCE` as `CHANGES_REQUIRED`.
