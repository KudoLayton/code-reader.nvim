...
11. Ask one read-only subagent to perform `Page Scope Review v1` from `../../references/code-reader-markdown-format.md`, including the hunk-to-page coverage table, any `hunk_fallback` evidence, and a `focus_alignment` record for every Diff page. For every `fallback_required` page, it must measure V(G), independent concepts, and variable--value pairs with the same split thresholds. Treat `MISMATCH` or `INSUFFICIENT_EVIDENCE` as `CHANGES_REQUIRED`.
