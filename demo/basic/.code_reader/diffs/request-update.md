...
After reading, the reviewer can follow the changed request lifecycle and verify why successful write-style requests now return a created response.

## Representative example

A `PUT` request that was previously rejected now passes validation, keeps its path and request id, then receives a `201` response.

## Architecture and reading flow

Use this demo in execution order to inspect the side-by-side diff view, gutter markers, inline modified spans, moved-line detection, and changed-line coverage.
